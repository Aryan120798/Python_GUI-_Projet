...
class allorderdet():
    def allorder(self):
        ...
        root33.geometry("700x400")

        frame44 = Frame(root33, width=900, height=450)
        frame44.pack()
        scrollbar = Scrollbar(frame44)
        scrollbar.pack(side=RIGHT, padx=10, ipady=50, expand=TRUE)

        mylist = Listbox(frame44, yscrollcommand=scrollbar.set)
        ...
        label_3 = Label(frame44, font=("Poiret One", 10), fg='#34495E')
        ...
        for i in range(1, len(list1)):
            ...
            y = list2[0] + " " * (19 - var1) + list2[1] + " " * (18 - var2) + list2[2] + " " * (15 - var3) + list2[
                3]
            print('ikuji', y)
        ...
        backbutton = Button(frame44, image=tback, relief=FLAT, command=t2)
        backbutton.place(x=2, y=300)
        root33.resizable(width=FALSE, height=FALSE)
        root33.mainloop()
...
class testframe:
    ...
    def testframefun(self):
        ...
        screen_height = root22.winfo_screenheight()
        screen_width = root22.winfo_screenwidth()

        x_coordinate = (screen_width / 2) - (window_width / 2)
        y_coordinate = (screen_height / 2) - (window_height / 2)

        root22.geometry('%dx%d+%d+%d' % (window_width, window_height, x_coordinate, y_coordinate))

        f1 = Frame(root22, height='900', width='400')

        orderidl= Label(f1, text='Order id :', font=("Poiret One", 16), fg='#000000')
        orderidl.place(x=0, y=27)

        orderidno = Label(f1, font=("Poiret One", 16), fg='#000000')
        orderidno.place(x=100, y=27)

        name = PhotoImage(file="label.png")
        tname = name.subsample(3, 3)

        l1 = Label(f1, image=tname, font=("Poiret One", 16), fg='#000000')
        l1.place(x=0, y=60)

        phone = PhotoImage(file="telephone.png")
        tphone = phone.subsample(3, 3)

        l2 = Label(f1, font=("Poiret One", 16), fg='#000000', image=tphone)
        l2.place(x=0, y=100)

        address = PhotoImage(file="placeholder.png")
        taddress= address.subsample(3, 3)

        l3 = Label(f1, image=taddress, font=("Poiret One", 16), fg='#000000')
        l3.place(x=0, y=140)
    ...
    def staffmain(self):
        ...
        def deletedata():
            ...
            if q is 1:
                ...
                conn.commit()
                c.execute("DELETE from ORDER_DETAILS WHERE Order_id=?", (x,))
                conn.commit()
        ...
        def t4():
            ...
            if q is 1:
                random1 = counters.valueee(x)
                root4.destroy()
                allorderdet.allorder(self)
        ...
        showorderdet = Button(frame5, text="Show Order Details", width=16,command=t4)  # TODO link login button
    ...
    def canorder(self):
        ...
        def deletedata():
            ...
            def cal(a):
                conn = sqlite3.connect('Main.db')
                c = conn.cursor()
                c.execute("SELECT Order_id from ORDER_DETAILS")
                x1 = c.fetchall()
                length = len(x1)
                d1 = ''.join(map(str, x1))
                a = d1.replace(',', '')
        ...
        deletebut = Button(frame4, text="Delete", width=13,command=deletedata)  #TODO link login button
        deletebut.place(x=430, y=42)
